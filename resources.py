...
class File(Resource):
    def get(self, filename):
        ...
        if file_data:
            columns_data = ColumnStatisticsModel.query.filter_by(filename=filename).order_by(ColumnStatisticsModel.column_name).all()
            return {'file': file_data.to_json(), 'columns': [column.to_json() for column in columns_data]}, 200
    ...
    def post(self, filename):
        ...
        file = FileModel(filename=filename, n_columns=file_column_numbers, n_rows=file_row_numbers)
        db.session.add(file)

        columns_to_return = []
        for column_name in csv_file.columns:
            column_statistics = ColumnStatisticsModel(column_name=column_name, filename=filename, 
                                                      ...
                                                      percent_of_missing_values=float(csv_file[column_name].isnull().sum() / file_row_numbers))
            columns_to_return.append(column_statistics)
            db.session.add(column_statistics)
        
        
        db.session.commit()
        return {'file': file.to_json(), 'columns': [column.to_json() for column in columns_to_return]}, 201
...
class Statistics(Resource):
    def get(self, filename, statistic_name):
        ...
        if file_data:
            try:
                columns_data = ColumnStatisticsModel.query.filter_by(filename=filename).order_by(ColumnStatisticsModel.column_name).all()
                return {'file': file_data.to_json(), 'columns': [{'column_name' : getattr(column, 'column_name'), statistic_name : getattr(column, statistic_name) } for column in columns_data]}, 200
